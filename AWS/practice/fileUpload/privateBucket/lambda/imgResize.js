...
exports.handler = async (event, context, callback) => {
  ...
  const Key = event.Records[0].s3.object.key; // 업로드된 키명 test/00310_HD.jpg
  const s3obj = { Bucket, Key };

  const filename = Key.split("/")[Key.split("/").length - 1]; // 경로 없애고 뒤의 파일명만 00310_HD.jpg
  const ext = Key.split(".")[Key.split(".").length - 1].toLowerCase(); // 파일 확장자만 jpg
